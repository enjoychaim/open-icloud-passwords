...
console.log("[Open iCloud Passwords for Chrome] content script v0.45.0 loaded");
...
function buildSuggestionBox(field) {
  ...
  box.setAttribute("aria-label", "Open iCloud Passwords for Chrome suggestions");
  glassify(box);
  ...
  header.textContent = "Open iCloud Passwords for Chrome";
...
async function maybeOfferSave(scope) {
  ...
  if (
    ...
    console.debug("[Open iCloud Passwords for Chrome] save skipped: recently autofilled");
    return;
  }
  ...
  console.debug("[Open iCloud Passwords for Chrome] handing save to background", {
